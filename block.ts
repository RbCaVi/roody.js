...
class NormalBlock extends Block {
  ...
  static async create(file,offset=0){
    const image=crop(await canvas.loadImage(`blocks/${file}.png`),[offset,0,offset+32,32]);
  ...
  draw(welded: Welded,rotate: Rotate=0,size: number=128){
    ...
    for(const [x,xside] of [[0,left],[8,right]]){
      for(const [y,yside] of [[0,top],[8,bottom]]){
        im.getContext('2d').drawImage(this.image,x+16*xside,y+16*yside,x+16*xside+8,y+16*yside+8,x,y,8,8);
      }
    }
...
function rotateblock(im: canvas.Canvas | canvas.Image,rotate){
  if(rotate==0){
    return im;
  }
  ...
  if(rotate==3){
    ctx.save();
    ctx.setTransform(new DOMMatrix([0,-1,1,0,0,im.height]));
    ctx.drawImage(im,0,0);
    ctx.restore();
  }
  if(rotate==1){
    ctx.save();
    ctx.setTransform(new DOMMatrix([0,1,1,0,0,0]));
    ctx.drawImage(im,0,0);
    ctx.restore();
  }
  if(rotate==2){
    ctx.save();
    ctx.setTransform(new DOMMatrix([1,0,0,-1,0,im.height]));
    ctx.drawImage(im,0,0);
    ctx.restore();
  }
  return im2;
...
class NoWeldBlock extends Block {
  ...
  static async create(file){
    const image=crop(await canvas.loadImage(`blocks/noweld/${file}.png`),[0,0,16,16]);
...
class WaferBlock extends Block {
  ...
  static async create(file,base="wafer",offset=0){
    const wafer=crop(await canvas.loadImage(`blocks/${base}.png`),[0,0,32,32]);
    const wire=crop(await canvas.loadImage("blocks/wire.png"),[offset,0,offset+32,32]);
    const image=crop(await canvas.loadImage(`blocks/${base}/${file}.png`),[offset,0,offset+32,32]);
  ...
  draw(welded: Welded,rotate: Rotate=0,size: number=128,offset=[0,0]){
    const [top,left,bottom,right]=rotatewelded(welded,rotate);
    const im=canvas.createCanvas(16,16);
    for(const [x,xside] of [[0,left],[8,right]]){
      for(const [y,yside] of [[0,top],[8,bottom]]){
        ...
        im.getContext('2d').drawImage(this.wafer,x+xoff,y+yoff,x+xoff+8,y+yoff+8,x,y,8,8);
      }
    }
    for(const [x,xside] of [[0,left],[8,right]]){
      for(const [y,yside] of [[0,top],[8,bottom]]){
        ...
        im.getContext('2d').drawImage(this.wire,x+xoff,y+yoff,x+xoff+8,y+yoff+8,x,y,8,8);
      }
    }
    ...
    ctx.drawImage(this.image,16*offset[0],16*offset[1],16*(offset[0]+1),16*(offset[1]+1),0,0,16,16);
    ctx.restore();
    return resize(im,size);
  }
}
...
class WireBlock extends Block {
  ...
  static async create(base,offset=0){
    const wafer=crop(await canvas.loadImage(`blocks/${base}.png`),[0,0,32,32]);
    const image=crop(await canvas.loadImage("blocks/wire.png"),[offset,0,offset+32,32]);
  ...
  draw(welded: Welded,rotate: Rotate=0,size: number=128,offset=[0,0]){
    ...
    for(const [x,xside] of [[0,left],[8,right]]){
      for(const [y,yside] of [[0,top],[8,bottom]]){
        const xoff=(xside==2?16:0)+offset[0]
        const yoff=(yside==2?16:0)+offset[1]
        im.getContext('2d').drawImage(this.image,x+xoff,y+yoff,x+xoff+8,y+yoff+8,x,y,8,8);
      }
    }
    return resize(im,size);
  }
...
class PlatformBlock extends Block {
  ...
  draw(welded: Welded,_,size: number=128){
    ...
    for(const [x,xside] of [[0,left],[8,right]]){
      im.getContext('2d').drawImage(this.image,x+16*xside,y,x+16*xside+8,y+16,x,0,8,8);
    }
    return resize(im,size);
  }
}
...
class ActuatorBlock extends Block {
  ...
  head: canvas.Image
  static async create(){
    const base=crop(await canvas.loadImage("blocks/actuator_base.png"),[0,0,32,32]);
    const head=crop(await canvas.loadImage("blocks/actuator_head.png"),[0,0,32,32]);
  ...
  draw(welded: Welded,rotate: Rotate=0,size: number=128){
    const [headtop,baseleft,basebottom,baseright]=rotatewelded(welded,rotate);
    const [basetop,headleft,headbottom,headright]=[1,0,1,0];
    const im=canvas.createCanvas(16,16);
    for(const [x,xside] of [[0,headleft],[8,headright]]){
      for(const [y,yside] of [[0,headtop],[8,headbottom]]){
        im.getContext('2d').drawImage(this.head,x+16*xside,y+16*yside,x+16*xside+8,y+16*yside+8,x,y,8,8);
      }
    }
    for(const [x,xside] of [[0,baseleft],[8,baseright]]){
      for(const [y,yside] of [[0,basetop],[8,basebottom]]){
        im.getContext('2d').drawImage(this.base,x+16*xside,y+16*yside,x+16*xside+8,y+16*yside+8,x,y,8,8);
      }
    }
    const im2=rotateblock(im,rotate);
    return resize(im2,size);
...
function normalize(block){
  ...
  if(Array.isArray(block)){
    const out={"type":"air","rotate":0,"weld":"all"};
    block.map((x,i)=>{out[["type","rotate","weld"][i]]=x;});
    return out;
  }
  const out={"type":"air","rotate":0,"weld":"all"};
  Object.entries(block).map(([k,v])=>{out[k]=v;});
  return out;
}
...
const wafertypes=["accelerometer","capacitor","diode","galvanometer","latch","matcher","potentiometer","sensor","transistor","wire_board"];
const wiretypes=["detector","port","toggler","trigger","wire"];
const wiredtypes=["lamp","combiner"].concat(wafertypes,wiretypes);
const noweldtypes=["copper_ore","iron_ore","pulp","sand","silicon","spawner","telecross","air"];
...
function canweld(side,block){
  ...
  if(block["type"] in noweldtypes){
    return false;
  }	else if(block["type"] in ["cap","flower_magenta","flower_yellow","grass","motor","pedestal","spikes"]){
    sides=[false,false,true,false];
  }else if(block["type"] in ["actuator_head","wire_spool","telewall"]){
    // no sides
    sides=[true,false,true,false];
  }else if(block["type"] in ["combiner","extractor","injector","platform"]){
    // no top/bottom
    sides=[false,true,false,true];
  }else if(block["type"] in ["arc_furnace","beam_core","collector","creator","destroyer","dismantler","magnet","manipulator","mantler","teleportore"]){
    // no top
...
async function makeimage(blocks,bsize=128,autoweld=true){
  ...
  for(let xi=0;xi<xsize;xi++){
    for(let yi=0;yi<ysize;yi++){
      ...
      if(autoweld){
        const weldright=canweld("right",block)&&canweld("left",get(newblocks,xi+1,yi));
        const weldleft=canweld("left",block)&&canweld("right",get(newblocks,xi-1,yi));
        const weldbottom=canweld("bottom",block)&&canweld("top",get(newblocks,xi,yi+1));
        const weldtop=canweld("top",block)&&canweld("bottom",get(newblocks,xi,yi-1));
        block["weld"]=block["weld"].map((e,i)=>e&&[weldtop,weldleft,weldbottom,weldright][i]);
      }
      let b;
      if(block["type"]=="wire_board"||block["type"]=="wire"){
        ...
        block["weld"][1]=block["weld"][1]&&(get(newblocks,xi-1,yi)["type"]!="platform"?2:true);
        block["weld"][3]=block["weld"][3]&&(get(newblocks,xi+1,yi)["type"]!="platform"?2:true);
      ...
      if(block["type"] in wafertypes.concat(wiretypes)){
        block["weld"][0]=block["weld"][0]&&(get(newblocks,xi,yi-1)["type"] in wiredtypes?2:true);
        block["weld"][1]=block["weld"][1]&&(get(newblocks,xi-1,yi)["type"] in wiredtypes?2:true);
        block["weld"][2]=block["weld"][2]&&(get(newblocks,xi,yi+1)["type"] in wiredtypes?2:true);
        block["weld"][3]=block["weld"][3]&&(get(newblocks,xi+1,yi)["type"] in wiredtypes?2:true);
